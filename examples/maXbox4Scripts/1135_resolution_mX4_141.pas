unit Uvisualcifar10test_mX4_14_super_resolution;
//task: must read tiny iimage in file
...
Const //PReModel_NN = 'SimpleSeparableImageClassifier124_50_21.nn'; 
      ...
      //PReModel_NN = '..\examples\SimpleImageClassifierEkon26_70.nn';
        ...
        PReModel_NN_Res = '.\examples\super-resolution-7-64-sep.nn';
...
// need PReModel_NN_Res = '.\examples\super-resolution-7-64-sep.nn';
 ...
 const
  csExampleBaseFileName = 'super-resolution-7-64-sep';
...
begin
  ...
  Volume := TNNetVolume.Create();
  writeln(botostr(LoadImageFromFileIntoVolume('data_batch_1.bin', volume)));
  LoadNNetVolumeIntoTinyImage4(volume, img); //overload;
  I := 1;
  //volume.size

  //while not EOF(cifarFile) do begin
    //Read(cifarFile, Img);
    //LoadTinyImageIntoNNetVolume(var TI: TTinyImage; Vol: TNNetVolume); overload;
    //Volume := TNNetVolume.Create();
    LoadTinyImageIntoNNetVolume1(img, volume); //overload;
  ...
  WriteLn(' Layers: '+itoa( NN4.CountLayers()  ));
  WriteLn(' Neurons:'+itoa(NN4.CountNeurons() ));
  WriteLn(' Weights:'+itoa(NN4.CountWeights() ));
  WriteLn('Computing...');
  ...
  while not (FWantQuit) do begin
    ...
    Label1.Caption := csTinyImageLabel[ ImgVolumes[I].Tag ];     //fix to tiny init!
    ...
    vDisplay.Add(2);
    vDisplay.Mul26(64);
    LoadVolumeIntoTImage(vDisplay, ImageGray, csEncodeRGB);
    ImageGray.Width := 128;
    ImageGray.Height := 128;
    ...
    vDisplay.Add(2);
    vDisplay.Mul26(64);
    LoadVolumeIntoTImage(vDisplay, ImageHE, csEncodeRGB);
    ImageHE.Width := 128;
    ImageHE.Height := 128;

    NN4.Compute65(vBigOutput,0);  NN4.GetOutput(vBigOutput);
...
begin
  ...
  //Data := Base64EncodeStr(BytesOf(AHeader)) + Ord('.') + Base64Encodestr(BytesOf(APayload));
  Data := bytesof(Base64EncodeStr((AHeader)) + itoa(ord('.')) + Base64Encodestr((APayload)));
  //if TgoSSLHelper.Sign_RSASHA256(Data, APrivateKey, JWS) then begin  
  //if RSAEncryptStr(Data, APrivateKey, JWS) then begin                                                               
    AJWT := StringOf(Data) + '.' + (Base64EncodeStr(stringof(JWS)));
    Result := True;
  //end
  //else
    Result := False;
...
begin //@main
 ...
 loadCifar10Dataset2(ImgVolumes, 1, 0,csEncodeRGB);
 
 //Volume := TNNetVolume.Create();
  //writeln(botostr(LoadImageFromFileIntoVolume('data_batch_1.bin', volume)));
  for it:= 1 to 100 do begin
    LoadNNetVolumeIntoTinyImage4(imgvolumes[it], img); //overload;   
    writeln(csTinyImageLabel[Img.bLabel]);
  end;  
  volume.free;  
  ImgVolumes.Free;                    
 end.

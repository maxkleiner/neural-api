...
begin
  ...
  result.AddSuperResolution({pSizeX=}SizeX, {pSizeY=}SizeY,
  ...
  LoadResizingWeights(result, FileName, true);
  //Result := NN;
  //nn.free;
end;
...
begin
  ...
  if FileExists('./model/'+FileName) then begin
    NN.LoadDataFromFile('./model/'+FileName);
    WriteLn('Neural network file found at ./model : '+ FileName);
  end else begin
...
begin
  ...
  writeln('Loading Tiny Cifar Images...');
   ...
   loadCifar10Dataset6(ImgVolumes, 1, 0,csEncodeRGB);   //1 is data_batch_1.bin
   writeln('tot size1 '+itoa(ImgVolumes.GetTotalSize( ))+ ' '+
                                                 itoa(imgvolumes.count));    //*)
   //TranslateCifar10VolumesToMachineAnimal(ImgVolumes);                                              
                                                 
   //CreateCifar10Volumes(ImgVolumes,ImgVolumes, ImgVolumes, csencodergb);                                              
    for I:= 2000 to 9999 do begin
        ImgVolumes.items[i].resize(0,0,0);  
        ImgVolumes.items[i].free; 
        //ImgVolumes.items[i]:= Nil; 
     end;    
    writeln('tot size2 '+itoa(ImgVolumes.GetTotalSize( ))+ ' '+
                                                 itoa(imgvolumes.count));
 ...
 //try
     ...
     try
       ...
       writeln('mem err in create resize..');
     end;  
     //NN2 := CreateResizingNN(32, 32, csExampleFileName);
       
   try  
   //AssignFile(cifarFile, 'data_batch_1.bin');
   //for I:= 100 to 5999 do 
     //   ImgVolumes.delete(imgvolumes[i]);
  ...
  WriteLn(' Layers: '+itoa( NN41.CountLayers()  ));
  WriteLn(' Neurons:'+itoa(NN41.CountNeurons() ));
  WriteLn(' Weights:'+itoa(NN41.CountWeights() ));
...
begin //@main
 ...
 (* NN:= THistoricalNets.Create();
  NN.AddSuperResolution({pSizeX=}128, {pSizeY=}128,
    {BottleNeck=}csExampleBottleNeck, {pNeurons=}csExampleNeuronCount,
    {pLayerCnt=}csExampleLayerCount, {IsSeparable=}csExampleIsSeparable);
  //LoadResizingWeights(NN, FileName, true);
  LoadResizingWeights(NN, csExampleFileName, true);
  writeln(objtostr(nn));
  nn.free;  *)
  
  nn:= CreateResizingNN(128, 128, csExampleFileName);
  writeln(objtostr(nn));
  writeln('layers '+itoa(nn.CountLayers()));
  nn.free;
  
  {NN3:= CreateResizingNN(64, 64, csExampleFileName);
  writeln(objtostr(nn3));
  nn3.free; }
